...
1. tres estados completos del mismo aeropuerto (`A → B → C`) muestran que los
   pasajeros del fondo avanzaron y que el grupo cambió;
...
permanece intacto y funciona como respaldo si faltan B, C o los pasajeros.

La capa se monta en este orden:

```text
AirportBackground (A/B/C) → AirportAmbience → AirportAgent → interfaz DOM
```
...
Los estados B y C se generaron con ImageGen integrado. B parte del fondo
original y cambia sólo los pasajeros. C parte de B y continúa el avance. PixiJS
...
## Línea temporal A → B → C

Cada estado se mantiene 4,8 segundos y se disuelve al siguiente durante 1,2
segundos con `smoothstep`. El ciclo completo dura 18 segundos:

```text
A (original) → B (avance 1) → C (avance 2) → A
```

Los tres sprites ocupan el mismo contenedor. Si sólo carga A, el modo se degrada
a `static`. Si carga una sola variante, el mismo cálculo funciona con dos
estados. La función pura está cubierta por `tests/backgroundMotion.test.ts`.
...
- tres fondos cargados en modo `state-dissolve`;
